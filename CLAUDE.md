...
4. **Scene / THEMES / ANOMALIES** — 本作の核。`THEMES`(3種: dusk/morning/twilight)は空・壁・遠景の
   ...
   **THEMES は ANOMALIES が触る値（のれん・看板・提灯の点灯・扉・臼の湯気・ポスター色・団子の色・
   ...
   テーマを増やすときも同じ制約を守ること。`THEMES[].label` に日本語の表示名を持たせ、
   `renderScene()` 最後の `drawThemeLabel()` でキャンバス右上に小さいチップとして常時表示する
   （「テーマの名前をどこかに小さく」という要望、2026-09-23）。空の色に関わらず読めるよう
   暗い半透明チップ+明るい文字の固定スタイルにしてある。出口看板（x=480-590,y=44-84）より
   さらに上（y=8-26）に置いてあり重ならない。HTML オーバーレイではなく canvas 直描きなので
   `renderOnly()` の差分検証にもそのまま乗るが、全テーマ・全異変で同じ位置に同じ文字が乗るだけ
   のため視認性テストには影響しない（3テーマ×21異変で再確認済み、`invisible=[]`）。
